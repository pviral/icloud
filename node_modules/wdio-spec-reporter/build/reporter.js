...
var SpecReporter = (function (_events$EventEmitter) {
    ...
    _createClass(SpecReporter, [{
        key: 'indent',
        value: function indent(cid, specTitle) {
            var indents = this.suiteIndents[cid] && this.suiteIndents[cid][specTitle] || -1;
        ...
        value: function getResultList(cid, suites) {
            ...
            for (var specTitle in suites) {
                var spec = suites[specTitle];
                var indent = this.indent(cid, specTitle);
                output += preface + '   ' + indent + specTitle + '\n';

                for (var testTitle in spec.tests) {
                    var test = spec.tests[testTitle];
